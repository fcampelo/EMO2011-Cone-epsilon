...
%
% (i)  : eMOEA - Steady-state MOEA based on eps-dominance concept
% (ii) : Simulated Binary Crossover (SBX)
% (iii): Polynomial Mutation (PM)
%
% Exemplos: [A,jA] = eMOEA(@deb52)
%           [A,jA] = eMOEA(@dtlz2)
%
...
%% Atualiza a população de memória usando eps-não-dominânica
Ax   = [];
jAx  = [];
boxA = [];
for i = 1:np    
    [Ax,jAx,boxA] = updade_archive(Ax,jAx,boxA,P(:,i),jP(:,i),boxP(:,i));
end
A(g).mem  = Ax;
jA(g).mem = jAx;
...
while (NEF <= NEFmax)
    ...
    % Atualiza a elite      
    for i = 1:2
        [Ax,jAx,boxA] = updade_archive(Ax,jAx,boxA,Q(:,i),jQ(:,i),boxQ(:,i));
    end
